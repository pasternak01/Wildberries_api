...
def get_main_table():
    ...
    df1 = df[['payment_justification', 'sale_date', 'quantity', 'wb_commision_in_rub', 'retail_price', 'kvv_percent',
              'acquiring_payment_commission', 'amount_to_seller', 'delivery_services']]

    table_summary = df1.groupby(['quantity', "payment_justification"]).agg(
        qty_operations=('payment_justification', 'count'), total_price=('retail_price', 'sum'),
        wb_comission_rub=('wb_commision_in_rub', 'sum'), acquring_sum=('acquiring_payment_commission', 'sum'),
        delivery_sum=('delivery_services', 'sum'), seller_profit=("amount_to_seller", 'sum'))
